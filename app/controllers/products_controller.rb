class ProductsController < ApplicationController
  ...
  def etichette
    ...
    format.html
    format.pdf do
      render :pdf => @products.count.to_s , orientation: "portrait",page_size: 'A4',margin:  {top:'0mm',bottom: '0mm',left: '0mm',right:'0mm' }, disable_smart_shrinking: true, show_as_html: params.key?('debug')
  ...
  def create
    ...
    respond_to do |format|
      if @product.save
        format.html { redirect_to utilities_etichette_path, notice: "Aggiunta." }
  ...
  def update
    respond_to do |format|
      if @product.update(product_params)
        format.html { redirect_to utilities_etichette_path(:id => @product.id), notice: "Aggiornato." }
