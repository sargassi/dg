class UtilitiesController < ApplicationController
  ...
  def etichette_check
    ...
    @group = Eticheck.last.present? ? Eticheck.last.group.to_i + 1 : 1

    @etigroup = if params[:group].present? && params[:group] != ''
        Eticheck.where('group = ?', params[:group]).order('created_at DESC').group_by { |x| x.group }
      else
        Eticheck.all.order('created_at DESC').group_by { |x| x.group }
      end
  ...
  def eticheckimp
    if params[:eticheck].present? && params[:eticheck] == 'y'
      gen = ImportLabCheckService.new
      gen.call(params[:file])
    end
    respond_to do |format|
      format.html { redirect_to utilities_etichette_check_path(eticheck: 'y'), notice: 'Ok.' }
      format.json { head :no_content }
    end
  end
